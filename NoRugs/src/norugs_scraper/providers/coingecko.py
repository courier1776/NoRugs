...
class CoinGeckoProvider:
    ...
    def collect(self, coin_ids: list[str]) -> int:
        ...
        with self.db.connect() as conn:
            ...
            try:
                # A SAVEPOINT (via conn.transaction()) means a failure here
                # only rolls back this block, not the whole connection. If we
                # skip this and something below raises, Postgres aborts the
                # entire transaction and the finish_run() write in the except
                # branch would itself fail with "current transaction is
                # aborted" — masking whatever actually went wrong.
