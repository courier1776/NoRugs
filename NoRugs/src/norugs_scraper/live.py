...
class UpdateState:
    ...
    latest_errors: dict[str, str] = field(default_factory=dict)
...
class LiveUpdateService:
    ...
    def run_once(self) -> bool:
        ...
        try:
            raw_results = self.collector(self.settings, self.config_path, provider="all")
            counts = {name: info["count"] for name, info in raw_results.items()}
            errors = {name: info["error"] for name, info in raw_results.items() if info["error"]}
            total_collected = sum(counts.values())
            completed = utc_now_iso()
            with self._state_lock:
                self._state.latest_counts = counts
                self._state.latest_errors = errors
                self._state.last_completed_at = completed
                self._state.cycles_completed += 1
                if total_collected > 0:
                    # At least one provider actually wrote data — treat as a
                    # real success even if some other providers failed.
                    self._state.last_success_at = completed
                    self._state.last_error = (
                        f"Partial failure: {', '.join(errors)} produced no data this cycle."
                        if errors
                        else None
                    )
                else:
                    # Every provider failed or had nothing to report — do NOT
                    # touch last_success_at, and surface why so the dashboard
                    # doesn't claim updates are 'verified' when nothing landed
                    # in the database.
                    self._state.last_error = (
                        "; ".join(f"{name}: {msg}" for name, msg in errors.items())
                        if errors
                        else "No records were returned by any provider this cycle."
                    )
            LOG.info("Live update completed: %s", counts)
            return not errors or total_collected > 0
