...
|---|---|---|
| Python 3.13.x (recommended) | Runs the backend server and data collector | https://python.org/downloads |
...
### Windows
1. Go to https://python.org/downloads and download the latest **Python 3.13.x** installer.
...
> **Note on Python versions:** This project supports Python **3.11 through 3.14**. Earlier versions of this guide recommended avoiding Python 3.14 because `pydantic`/`psycopg` didn't yet ship 3.14 wheels — that's no longer the case, and the pins in `pyproject.toml`/`requirements.txt` are kept up to date to track it. If you ever see `pip` trying to **build a wheel from source** (long compiler output, `maturin`/`cargo` errors, "Failed building wheel for pydantic-core") on a fresh install, it means the pinned version is stale for your Python version — check for newer `pydantic`/`pydantic-settings` releases on PyPI and bump the pins in both files rather than downgrading Python.
...
2. Paste in the following, replacing the placeholders with the values from Step 2:
   ...
   # Optional live data providers — the app works without any of these,
   # using public endpoints. See "No API keys needed" below for why.
   COINGECKO_API_KEY=
   COINGECKO_PLAN=demo
   ETHERSCAN_API_KEY=
   GITHUB_TOKEN=

   # Keeps every provider inside free/unauthenticated rate limits without
   # requiring API keys. Raise this only if you add paid API keys above.
...
The app is designed to run entirely on public, unauthenticated endpoints — you do not need to sign up for anything to get live data. Two settings make this safe:

- **`MAX_TRACKED_ASSETS`** (default `50`) caps how many entries from every list in `config/assets.json` get collected, so the app never scans more than a token-free amount of data.
- **`GITHUB_REFRESH_INTERVAL_SECONDS`** / **`GITHUB_MAX_REPOS_PER_RUN`** specifically throttle GitHub developer-activity collection. GitHub's *unauthenticated* API allows only 60 requests/hour, and this app makes 4 requests per tracked repo (repo info, commits, contributors, closed issues) — so even the default 50 tracked assets would blow through that budget instantly if collected all at once. Instead, the app automatically refreshes only a rotating batch of `GITHUB_MAX_REPOS_PER_RUN` repos (default 10 → 40 requests) once per `GITHUB_REFRESH_INTERVAL_SECONDS` (default 3600 = 1 hour), cycling through the full list over time. With the defaults, 50 tracked repos fully rotate every 5 hours, always staying under GitHub's limit.

You can leave `GITHUB_TOKEN` blank indefinitely under normal use. If you raise `MAX_TRACKED_ASSETS` well above 50 *and* want fresher GitHub data than the rotation provides, adding a token removes the 60/hour ceiling (see below) — but it's an optimization, not a requirement.

### (Optional) Getting free API keys

Adding free keys raises rate limits and improves reliability/freshness, but nothing in this app requires them:
...
- **GitHub** — https://github.com/settings/tokens → generate a personal access token (no special scopes needed for public repo data). Paste it into `GITHUB_TOKEN`. Only useful if you also raise `GITHUB_MAX_REPOS_PER_RUN`/lower `GITHUB_REFRESH_INTERVAL_SECONDS`, since the defaults are already designed to never need it.

**Never commit a real key/token into `README.md`, `config/assets.json`, or anything else that isn't `.env`** — `.env` is the only file meant to hold live credentials, and it's excluded from version control for that reason.
...
**`ImportError: no pq wrapper available` / `psycopg 'binary' implementation: No module named 'psycopg_binary'`**
This means `psycopg` got installed without its binary driver, which also requires the `libpq` C library to be present on your system (usually missing on Windows). This shouldn't happen if you followed Step 7 exactly — both `pip install -e .` and `pip install -r requirements.txt` pull in `psycopg[binary,pool]`, which bundles everything needed. If you still hit this, someone/something (a tutorial, a stale lockfile, an old cached wheel) likely installed plain `psycopg` on top. Fix it with:
```bash
pip uninstall psycopg
pip install "psycopg[binary,pool]>=3.3,<4"
```
Then re-run Step 7 to confirm, and verify with:
```bash
python -c "import psycopg; print(psycopg.__version__)"
```

**`ERROR: Failed building wheel for pydantic-core`**
This means pip is trying to **compile** `pydantic-core` from source instead of downloading a prebuilt wheel — which happens when your Python version is newer than the pinned `pydantic` version supports. `pydantic-core` is written in Rust and has no reliable source-build path on Windows without a full Rust toolchain, so this always needs a version bump rather than a local fix. Check `python --version`, then check https://pypi.org/project/pydantic/#history for a release that lists your Python version as supported, and update the `pydantic`/`pydantic-settings` lines in both `pyproject.toml` and `requirements.txt` to match — keep the two files' versions identical. Then reinstall:
```bash
pip install -e .
```

**`github: Client error '403 rate limit exceeded'` in the live-update panel**
This means GitHub's unauthenticated 60-requests/hour limit was hit — normally prevented automatically by the `GITHUB_REFRESH_INTERVAL_SECONDS`/`GITHUB_MAX_REPOS_PER_RUN` throttling described in Step 10. If you still see it:
- You likely lowered `GITHUB_REFRESH_INTERVAL_SECONDS` or raised `GITHUB_MAX_REPOS_PER_RUN`/`MAX_TRACKED_ASSETS` past what the unauthenticated budget supports (10 repos × 4 requests = 40/hour is the safe default headroom).
- Multiple instances of the app are running at once and both are polling GitHub independently.
- Add a `GITHUB_TOKEN` (see "Getting free API keys" above) if you specifically want faster/larger GitHub coverage than the defaults allow — otherwise, just revert your settings to the defaults and the throttling will recover on its own within the hour.

**Clicking an asset on the dashboard shows "API unavailable" / "Unable to load asset"**
This almost always means the asset just isn't in the database yet — not a broken connection. Causes, in order of likelihood:
- The live collector hasn't scraped it yet. Wait ~30–60 seconds after starting the server, or run `norugs-scrape --provider all` manually (Step 12).
- The asset isn't listed in `config/assets.json`, so it will never appear no matter how long you wait.
- The database connection itself is down — check `/api/health` first; if that also shows an error, follow the `/api/health` troubleshooting entry above instead.
