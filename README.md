...
This reads `pyproject.toml`, installs the project and its dependencies, and registers the NoRugs package with your virtual environment.

> If this step fails for any reason, `python -m pip install -r requirements.txt` installs the same dependencies without registering those two commands — in that case, use `python run.py` instead of `python run.py` in the steps below.
...
This is the recommended launch command for all users.
...
**`command not found: python run.py`**
The package was not installed correctly or the virtual environment is not active. Run:
```bash
...
```bash
PORT=5001 python run.py        # macOS
set PORT=5001 && python run.py # Windows (Command Prompt)
```
