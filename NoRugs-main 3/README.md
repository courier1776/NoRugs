...
> **Important:** Python **3.14 is not currently recommended** for this project because some dependencies (particularly `psycopg[binary,pool]`) may not yet provide compatible builds. Installing Python **3.13.x** avoids these compatibility issues and is the version used for the installation instructions below.
...
Python wasn't installed correctly, the terminal needs to be restarted, or you're using Python 3.14. If Python 3.14 is installed, uninstall it and install Python 3.13.x before continuing. Close and reopen Command Prompt / Terminal and try again. On Windows, re-run the Python installer if `py --version` still fails. Note this only applies *before* Step 6 — after the virtual environment is created and activated, use plain `python` on both platforms, per the note in Step 6.
